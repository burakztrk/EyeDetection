...
class GooglyEyesGraphic extends GraphicOverlay.Graphic {
    ...
    private void drawEye(Canvas canvas, PointF eyePosition, float eyeRadius,
        ...
        if (isOpen) {
            canvas.drawCircle(eyePosition.x, eyePosition.y, 20, mEyeWhitesPaint);
//            canvas.drawCircle(irisPosition.x, irisPosition.y, irisRadius, mEyeIrisPaint);
        } else {
            canvas.drawCircle(eyePosition.x, eyePosition.y, 20, mEyeLidPaint);
            float y = eyePosition.y;
            float start = eyePosition.x - 5;
            float end = eyePosition.x + 5;
            canvas.drawLine(start, y, end, y, mEyeOutlinePaint);
        }
        canvas.drawCircle(eyePosition.x, eyePosition.y, 20, mEyeOutlinePaint);
    }
}